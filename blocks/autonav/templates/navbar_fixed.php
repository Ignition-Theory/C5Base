...
/* 	FIXED NAVIGATION BAR
   	==========================================================================
	Uses the Twitter Bootstrap Navbar component (http://twitter.github.com/bootstrap/components.html#navbar)
...
foreach ($navItems as $ni) {
	...
	if ($ni->isEnabled) {
		echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->anchorClasses . '"' . $dataToggle . '>' . $ni->name . $caret . '</a>';
	} else {
		echo '<span class="' . $ni->anchorClasses . '">' . $ni->name . '</span>';
	}
...
echo '</ul></div></div></div></nav>'; //closes the top-level menu