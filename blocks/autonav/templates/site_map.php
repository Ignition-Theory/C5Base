...
/*	SITE MAP
   	==========================================================================
   	Displays a multi-layer site map using CSS from Slickmap (http://astuteo.com/slickmap)
...
//loop through items to find number of top-level links
$topLevelItems = -1;
foreach ($navItems as $ni){
	if ($ni->level == 1){
		$topLevelItems++;
...
echo '<nav><ul class="siteMap col' . $topLevelItems . '">'; //opens the top-level menu
...
foreach ($navItems as $ni) {
	...
	if ($ni->isEnabled) {
		echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->anchorClasses . '">' . $ni->name . '</a>';
	} else {
