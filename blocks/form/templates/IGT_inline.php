...
while ($questionRow = $questionsRS->fetchRow()) {
	...
	if ($question['required']) {
		if ($question['type'] == 'select') {
			$search = '<select';
			$replace = '<select id="'.$domId.'" class="validate[required] input-small"';
			$question['input'] = str_replace($search, $replace, $question['input']);
		} else if ($question['type'] == 'textarea') {
			$search = '<textarea';
			$replace = '<textarea id="'.$domId.'" class="validate[required] input-small"';
			$question['input'] = str_replace($search, $replace, $question['input']);
			...
			if ($question['type'] == 'email'){
				$search = '<input';
				$replace = '<input id="'.$domId.'" class="validate[required,custom[email]] input-small"';
				$question['input'] = str_replace($search, $replace, $question['input']);
			} else if ($question['type'] == 'telephone'){
				$search = '<input';
				$replace = '<input id="'.$domId.'" class="validate[required,custom[phone]] input-small"';
				$question['input'] = str_replace($search, $replace, $question['input']);
			} else if ($question['type'] == 'url'){
				$search = '<input';
				$replace = '<input id="'.$domId.'" class="validate[required,custom[url]] input-small"';
				$question['input'] = str_replace($search, $replace, $question['input']);
			} else {
				$search = '<input';
				$replace = '<input id="'.$domId.'" class="validate[required] input-small"';
				$question['input'] = str_replace($search, $replace, $question['input']);
			}
		}
		...
		if ($question['type'] == 'email'){
			$search = '<input';
			$replace = '<input id="'.$domId.'" class="validate[custom[email]] input-small"';
			$question['input'] = str_replace($search, $replace, $question['input']);
		} else if ($question['type'] == 'telephone'){
			$search = '<input';
			$replace = '<input id="'.$domId.'" class="validate[custom[phone]] input-small"';
			$question['input'] = str_replace($search, $replace, $question['input']);
		} else if ($question['type'] == 'url'){
			$search = '<input';
			$replace = '<input id="'.$domId.'" class="validate[custom[url]] input-small"';
			$question['input'] = str_replace($search, $replace, $question['input']);
...
<form enctype="multipart/form-data" id="miniSurveyView<?php  echo $bID; ?>" class="form-inline" method="post" action="<?php  echo $formAction ?>">
